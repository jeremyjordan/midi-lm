...
from midi_lm.models.multitrack_music_transformer.generate import SOS_SEED, generate_from_seed
...
class LightningMultitrackMusicTransformer(pl.LightningModule):
    def __init__(
        self,
        network_config: MultitrackMusicTransformerConfig,
        optimizer_config: dict | None = None,
        lr_scheduler_config: dict | None = None,
    ) -> None:
        super().__init__()
        self.network = MultitrackMusicTransformer(network_config)
    ...
    def _shared_step(self, batch):
        ...
        total_loss = (
            losses["event_type_loss"]
            + losses["beat_loss"]
            + losses["position_loss"]
            + losses["pitch_loss"]
            + losses["duration_loss"]
            + losses["instrument_loss"]
        )
